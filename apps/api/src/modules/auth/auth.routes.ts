...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    sendSuccess(res, { accessToken: result.accessToken, userId: result.userId }, 201);
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    setRefreshCookie(res, result.refreshToken);
    sendSuccess(res, { accessToken: result.accessToken, userId: result.userId });
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    sendSuccess(res, { message: 'If the account exists, reset instructions will be sent' });
  })
);
...
router.post(
  '/reset-password',
  authLimiter,
  validate({ body: z.object({ token: z.string().min(32), password: z.string().min(12).max(128) }).strict() }),
  asyncHandler(async (req, res) => {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    await changeCustomerPassword(req.auth!.sub, req.body.currentPassword, req.body.newPassword);
    clearRefreshCookie(res);
    sendSuccess(res, { success: true });
  })
);
...
adminRouter.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      await writeAuditLog({
        adminId: result.adminId, action: 'admin.login', entityType: 'AdminUser', entityId: result.adminId,
      ...
      sendSuccess(res, { accessToken: result.accessToken, permissions: result.permissions });
    } catch (error) {
      await writeAuditLog({
        action: 'admin.login_failed', entityType: 'AdminUser',
        requestId: res.locals.requestId, ipAddress: req.ip, userAgent: req.get('user-agent'),
      });
      throw error;
    }
