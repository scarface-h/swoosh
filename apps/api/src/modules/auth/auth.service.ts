  ...
  await prisma.$transaction([
    ...
    prisma.notification.create({
      data: {
        ...
        payload: {
          url: `${env.STOREFRONT_URL}/verify-email?token=${verificationToken}`,
        },
      },
    }),
  ]);
...
export async function refreshCustomer(raw: string) {
  ...
  if (
    !user ||
    user.status !== "ACTIVE" ||
    user.sessionVersion !== rotated.sessionVersion
  ) {
...
export async function createPasswordReset(email: string) {
  ...
  await prisma.notification.create({
    data: {
      ...
      payload: { url: `${env.STOREFRONT_URL}/reset-password?token=${raw}` },
    },
  });
  ...
  await prisma.$transaction([
    ...
    prisma.notification.create({
      data: {
        ...
        recipient: user.email,
        subject: "Verify your email",
        payload: { url: `${env.STOREFRONT_URL}/verify-email?token=${raw}` },
      },
    }),
  ]);
}
...
export async function refreshAdmin(raw: string) {
  ...
  if (
    !admin ||
    admin.status !== "ACTIVE" ||
    admin.sessionVersion !== rotated.sessionVersion
  ) {
    throw AppError.unauthorized("Session is no longer valid");
  }
  const permissions = await adminPermissions(admin.id);
