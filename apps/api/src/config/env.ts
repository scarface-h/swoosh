...
const envSchema = z
  ...
  .superRefine((value, context) => {
    if (value.NODE_ENV === "production") {
      if (
        value.ACCESS_TOKEN_SECRET.includes("dev-") ||
        value.ACCESS_TOKEN_SECRET.length < 32
      ) {
        context.addIssue({
      ...
      if (
        value.REFRESH_TOKEN_SECRET.includes("dev-") ||
    ...
    if (value.SMTP_HOST && !value.EMAIL_FROM) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["EMAIL_FROM"],
        message: "EMAIL_FROM is required when SMTP is configured",
      });
    }
  });
